...
### PyTorch验证
```python
# ▶ CrossEntropy + Softmax 梯度验证
import torch
import torch.nn.functional as F

x = torch.randn(4, 10, requires_grad=True)  # logits
y = torch.tensor([2, 5, 0, 7])  # target indices
...
print(f"梯度匹配: {torch.allclose(x.grad, grad_check)}")  # True
print(f"x.grad:\n{x.grad}")
```
...
| 资源 | 链接 | 说明 |
|------|------|------|
| **The Matrix Calculus You Need For Deep Learning** | [arXiv:1802.01528](https://arxiv.org/abs/1802.01528) | ⭐必读！深度学习矩阵求导完全指南 |
| **The Matrix Cookbook** | [PDF](https://www.math.uwaterloo.ca/~wgilbert/31/HarderWilkiem.pdf) | 矩阵公式速查手册 |
| **CS231n Backpropagation** | [link](http://cs231n.stanford.edu/slides/2017/cs231n_2017_lecture6.pdf) | Stanford CNN课程反向传播讲义 |
