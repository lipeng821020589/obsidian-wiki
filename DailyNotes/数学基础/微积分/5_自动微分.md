...
jacobian_v, = torch.autograd.grad(
...
z.backward(create_graph=True)  # 创建可微分的图
print(x.grad)  # 一阶导：3x^2 = [3, 12]

# 二阶导
grad_grad = x.grad @ torch.ones_like(x)  # 展平梯度
grad_grad.backward()
print(x.grad)  # 二阶导：6x = [6, 12]
```
...
print(a.grad)  # 2
print(b.grad)  # None（但如果是中间节点，可通过retain_grad保存）
```
